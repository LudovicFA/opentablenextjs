...
export default function AuthModal({isSingin}: {isSingin: boolean}) {
  ...
  const renderContent = (signinContent: string, signupConten: string) => {
        return isSingin ? signinContent : signupConten
  }
          ...
          loading ? 
            ...
            (<div className="p-2 h-[600px]">
              ...
              <div className="m-auto">
                  ...
                  <AuthModalInputs inputs={inputs} handleChangeInput={handleChangeInput} isSignIn={isSingin}/>
                  <button className='bg-red-600 uppercase w-full text-white p-3 rounded text-sm mb-5 disabled:bg-gray-400'
                    disabled={disabled}
                    onClick={handleClick}
                  >
                    {renderContent('Sign In', 'Create Acccount')}
                  </button>
              </div>
            </div>)
          }
